...
class Trip extends Component {
  ...
  componentDidMount() {
    axios({
      ...
      .then(response => {this.setState({ trip: response.data.trip }); this.props.saveTrip(response.data.trip)})
      .catch(console.error);
  }
  ...
  deleteTrip(id) {
    axios({
      ...
      .then(() =>
        this.setState({
          shouldRedirect: true,
          redirectMessage: 'Successfully deleted.',
        })
      )
      ...
      <article>
        <div key={id} className="trip-card container">
          ...
          <center>
            ...
            <Link
              to={{ pathname: `/trip/${id}/expense-create`, state: { trip } }}
            >
              <Button>
                <AddCircle />
                Expense
            ...
            {this.state.trip.expenses.map(expense => (
              <li key={expense.id}>
                <span>
                  ...
                  <Button
                    onClick={() =>
                      this.deleteExpense(expense.trip_id, expense.id)
                    }
                  >
                    <Delete />
                  </Button>
            ...
            {this.state.trip.todos.map(todo => (
              <li key={todo.id}>
                <span>
                  {todo.title}
                  <Button
                    onClick={() => this.deleteTodo(todo.trip_id, todo.id)}
                  >
                    <Delete />
                  </Button>
                </span>
